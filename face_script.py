...
if __name__ == '__main__':
    ...
    for i, like in enumerate(main_likes):
        try:
            driver.execute_script("arguments[0].setAttribute('style', 'position:relative;z-index:99999')", like)
            time.sleep(1)
            driver.execute_script(
                "window.scrollTo(" + str(int(like.location['x'])) + "," + str(int(like.location['y'])) + ")"
            )
            time.sleep(1)
